...
未解出，想到对异常礼包进行预处理，后面思路出错，求优惠差值进行排序，利用贪心选取礼包！
...
- 时间复杂度：$O(n \times k \times m^n)$，其中 k 表示大礼包的数量，m 表示每种物品的需求量的可能情况数（等于最大需求量加 1），n 表示物品数量。我们最多需要处理 $m^n$个状态，每个状态需要遍历 kk 种大礼包的情况，每个大礼包需要遍历 n 种商品以检查大礼包是否可以购买。
