...
设 $s[j][i]$表示子数组 $[\textit{arr}[j], \textit{arr}[j+1], \cdots,\textit{arr}[i]]$ 的最小值，则可以推出所有连续子数组的最小值之和为 $\sum_{i=0}^{n-1} \limits \sum_{j=0}^{i} \limits s[j][i]$ 。对于每个以 i 为最右的子数组最小值之和为 $\sum_{j=0}^{i} \limits s[j][i]$ 。我们只需要求出以每个元素 $\textit{arr}[i] $为最右的子数组最小值之和，即可求出所有的子数组的最小值之和。每当我们减少 jj 时，子序列的最小值可能会有关联，事实上我们可以观察到 $s[j-1][i] = \min(s[j][i], \textit{arr}[j-1])$。
...
$$
上述序列的最小值分别为$ [3, 3, 2, 2, 2, 1]$，可以发现重要点是 j = 5, j = 3, j = 0j，分别是 j 从 i 开始向左移动遇到的最小值的位置。如下图所示:
...
$$
\begin{aligned} \sum_{j=0}^{i} \limits s[j][i] & = \sum_{j=0}^{i - k} \limits s[j][i] + \sum_{j=i-k+1}^{i} \limits s[j][i] \\ & = \sum_{j=0}^{i - k} \limits s[j][i] + k \times \textit{arr}[i] \\ & = \sum_{j=0}^{i - k} \limits s[j][i-k] + k \times \textit{arr}[i] \end{aligned}
$$
我们令 $\textit{dp}[i] = \sum_{j=0}^{i} \limits s[j][i]$[i]，则上述等式转换为：
$$
\textit{dp}[i] = \textit{dp}[i-k] + k \times \textit{arr}[i]
$$
我们维护一个单调栈，很容易求出元素 x 的左边第一个比它小的元素，即求出以 xx 为最右且最小的子序列的最大长度，子数组的最小值之和即为 $\sum_{i=0}^{n-1} \limits \textit{dp}[i]$ 。
