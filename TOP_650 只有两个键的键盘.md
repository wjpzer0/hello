...
> 给定初始值 n，我们希望通过若干次操作将其变为 1。每次操作我们可以选择 n 的一个大于 1 的因数 j，耗费 j 的代价并且将 n 减少为 $\dfrac{n}{j} $。在所有可行的操作序列中，总代价的最小值即为 f[n]。
...
$$
恒成立。也就是说，在达到最小的总代价时，我们将 n 拆分成了若干个素数的乘积。因此我们只需要对 n 进行质因数分解，统计所有质因数的和，即为最终的答案。
