...
class Solution:
    def shortestSubarray(self, nums: List[int], k: int) -> int:
        preNum = [0]
        for num in nums:
            preNum.append(preNum[-1] + num)
        ans = len(nums) + 1
        q = deque()
        for i, curNum in enumerate(preNum):
            while q and curNum - preNum[q[0]] >= k:
                ans = min(ans, i - q.popleft())
            while q and preNum[q[-1]] > curNum:
                q.pop()
            q.append(i)
