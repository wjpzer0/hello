...
没有仔细读题，二叉搜索树，想划水就没写！查看评论发现一个有趣的解法！理解python 的yield 和yield from的用法！
...
「二叉树的中序遍历」可以参考 TOP_94. 二叉树的中序遍历，具体地，我们使用迭代方法，这样可以在找到答案后停止，不需要遍历整棵树。
...
- 时间复杂度：预处理的时间复杂度为 O(N)，其中 NN 是树中结点的总数；我们需要遍历树中所有结点来统计以每个结点为根结点的子树的结点数。搜索的时间复杂度为 O(H)，其中 H 是树的高度；当树是平衡树时，时间复杂度取得最小值 $O(\log N)$；当树是线性树时，时间复杂度取得最大值 O(N)。
...
其中，将二叉搜索树转换为平衡二叉搜索树，可以参考 TOP_1382. 将二叉搜索树变平衡的官方题解。在插入和删除操作中维护平衡状态相对复杂，读者可以阅读下面的代码和注释，理解如何通过旋转和重组实现它。
