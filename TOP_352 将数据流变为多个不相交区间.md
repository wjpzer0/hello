...
class SummaryRanges:
    ...
    def getIntervals(self) -> List[List[int]]:
        ...
        for i in range(1, n):
            if self.ranges[i] == end + 1:
                end += 1
            elif self.ranges[i] == end:
                continue
            else:
...
- 情况一：如果存在一个区间 $[l, r]$，它完全包含 $\textit{val}$，即 $l \leq \textit{val} \leq$ ，那么在加入 $\textit{val}$ 之后，区间集合不会有任何变化；

- 情况二：如果存在一个区间 $[l, r]$，它的右边界 r「紧贴着」$\textit{val}$，即 $r + 1 = \textit{val}$，那么在加入 $\textit{val}$ 之后，该区间会从 $[l, r]$ 变为 $[l, r+1]$；

- 情况三：如果存在一个区间 $[l, r]$[l,r]，它的左边界 l「紧贴着」$\textit{val}$，即 $l - 1 = \textit{val}$，那么在加入 $\textit{val}$ 之后，该区间会从 $[l, r]$ 变为 $[l - 1, r]$；
...
> 对于给定的 $\textit{val}$，我们需要找到区间 $[l_0, r_0]$，使得 $l_0$是最大的满足 $l_0 \leq \textit{val}$ 的区间。同时，我们需要找到区间 $[l_1, r_1]$，使得 $l_1$是最小的满足$ l_1 > \textit{val}$ 的区间。
