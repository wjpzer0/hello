...
- $\textit{withdraw}$ 操作
  ...
  如果要进行操作的帐号不在已有的帐号中，即 $\textit{account} > n$，那么交易无效。如果账号 $\textit{account}$ 的余额小于 $\textit{money}$money，那么交易无效。交易有效时，我们将账号 $\textit{account}$ 的余额减少 $\textit{money}$。
