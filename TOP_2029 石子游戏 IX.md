...
未解出！想到了可以被3整除的数的个数有关，即个数的奇偶，但是没有针对性的分析！
...
由于玩家的目标是使得已经被移除的石子的价值总和不是 3 的倍数，因此我们可以把石子分成三类，它们的价值除以 33 的余数分别为 0, 1, 2。我们可以直接用 0, 1, 2 代表它们的价值，对应的石子数量分别为 $\textit{cnt}_0, \textit{cnt}_1, \textit{cnt}_2$。
