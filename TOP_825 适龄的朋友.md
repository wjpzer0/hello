...
提炼出来信息，想到了排序，但是仍然思考的是$O(n^2)$的解法，忽略了适用区间也会单调递增，总想着全局搜索！
...
这样一来，$[\textit{left}, \textit{right}]$ 就是满足年龄要求的 yy 的下标。需要注意的是，x 本身一定在 $[\textit{left}, \textit{right}]$ 区间内，因此 x 发送的好友请求数，即为$ [\textit{left}, \textit{right}]$ 区间的长度减去 1。

我们将每一个 x 对应的 $[\textit{left}, \textit{right}]$ 区间长度减去 1 进行累加，就可以得到最终的答案。

#### 复杂度分析

- 时间复杂度：$O(n \log n)$。排序需要的时间为 $O(n \log n)$，遍历所有的 $\textit{ages}[x]$ages[x] 以及使用双指针维护答案区间的时间复杂度为 O(n)。
