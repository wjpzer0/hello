...
未解出，有部分思路，但是代码出错，未找出bug！找到bug（服务器构成一个环，必要的话可能从第 0 个服务器开始继续找下一个空闲的服务器）没有完成该条件！
...
class Solution:
    def busiestServers(self, k: int, arrival: List[int], load: List[int]) -> List[int]:
        ...
        for i in range(n):
            ...
            diff = arr - state
            choice = True
            newList = [mid for mid in range(i % k, k)] + [mid for mid in range(i % k)]
            for j in newList:
                kList[j] = max(0, kList[j] - diff)
                if kList[j] == 0 and choice:
                    kList[j] = loa
                    ans[j] += 1
                    choice = False
            state = arr
